...
def accumulate_data():
    ...
    dataset_path = parent_directory + "/data/training/training-dataset"
    files = os.listdir(dataset_path)

    for single_file in files:
        if ".json" in single_file:
            file_name = os.path.join(dataset_path, single_file)
            print(file_name)

            with open(file_name, encoding="utf-8-sig") as f:
                json_data = json.load(f)
                data_set.extend(json_data)

    return data_set


def add_data_to_existing_dataset():
    data_set = accumulate_data()
    desc_list = set()
    technique_wise_data = defaultdict(list)
    for single_object in data_set:
        desc_list = set()
        for description_data in single_object["desc"]:
            desc_list.add(description_data["field_data"])

        #
        for single_technique in single_object["techniqueMapping"]:
            temp_technique_data = technique_wise_data.get(single_technique, None)
            if temp_technique_data == None:
                technique_wise_data[single_technique] = list(desc_list)
            else:
                temp_technique_data.extend(list(desc_list))
                technique_wise_data[single_technique] = temp_technique_data

        # print(technique_wise_data)
    return technique_wise_data


def update_analyzed_report_with_training_dataset():
    parent_directory = os.getcwd()
    analyzed_report_path = (
        parent_directory + "/data/training/archive/all_analyzed_reports_state_B.json"
    )

    technique_wise_data = add_data_to_existing_dataset()
    ...
    with open(
        ...
        for single_technique in technique_wise_data:
            if single_technique in ATTACK_LOOKUP.keys():
                ...
                existing_technique_data_in_report.extend(
                    technique_wise_data[single_technique]
                )
                all_analyzed_reports[technique_name] = list(
                    set(existing_technique_data_in_report)
                )

            else:
                print(single_technique)

    for technique_name, technique_data in all_analyzed_reports.items():
        if type(technique_data) == list:
            print(technique_name, " Sentence Count ", len(technique_data))

    with open(
        parent_directory + "/data/training/archive/all_analyzed_reports_12.json", "w"
    ) as f:
        json.dump(all_analyzed_reports, f, indent=4)


update_analyzed_report_with_training_dataset()
